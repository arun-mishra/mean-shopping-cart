...
exports.getIndex = (req, res, next) => {
  ...
  Product.find()
    ...
    .then(products => {
      ...
      //   res.status(200).sendFile((path.join(__dirname, '../', row.imageUrl)))
      // })
      res.status(201).json({
            products:products,
            currentPage: page,
...
exports.postMultiCart= (req,res,next)=>{
  ...
  .then(user=>{
    ...
    let userItem = user.cart.items;
    let cartItem = req.body;
    userItem.forEach(userRow=>{
      cartItem.forEach(itemRow=>{
        if(itemRow._id.toString() === userRow._id.toString()){
          userRow.quantity = itemRow.quantity
        }
      })
    })
    return userItem;
